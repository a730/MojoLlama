...
# ── Batched Q8_0 matmul (4-accumulator for B=4) ──
@always_inline("nodebug")
@always_inline
def _mm_q8_batch(q8addr: Int, x: UnsafePointer[Float32, MutExternalOrigin],
    ...
    comptime
    if B == 1:
        _mm_q8_single(q8addr, x, o, nr, nc, nw)
    elif B == 2:
        var q = UnsafePointer[UInt8, MutExternalOrigin](unsafe_from_address=Int(q8addr))
        var nb = (nr + RPW - 1) // RPW
        var rb = q8_row_bytes(nc)
        def wk2(wi: Int) capturing:
            var rs = wi * RPW; var re = rs + RPW
            if re > nr: re = nr
            for r in range(rs, re):
                var ro = r * rb
                var a0 = SIMD[DType.float32, W](0.0); var a1 = SIMD[DType.float32, W](0.0)
                var col = 0
                while col < nc:
                    var bo = ro + (col // QK) * QB; prefetch[](q + bo + QB)
                    var lo = Int(q.load(bo + 0)); var hi = Int(q.load(bo + 1))
                    var sv = SIMD[DType.float32, W](h2f(UInt16(lo | (hi << 8))))
                    comptime for grp in range(4):
                        var wf = (q.load[width=8](bo + 2 + grp * 8).cast[DType.float32]() - 128.0) * sv
                        a0 = wf.fma[FastMathFlag.FAST](x.load[width=W](0*nc + col + grp*8), a0)
                        a1 = wf.fma[FastMathFlag.FAST](x.load[width=W](1*nc + col + grp*8), a1)
                    col += QK
                o.store(0*nr + r, a0.reduce_add()); o.store(1*nr + r, a1.reduce_add())
        parallelize[func=wk2](num_work_items=nb, num_workers=nw)
    else:
        var q = UnsafePointer[UInt8, MutExternalOrigin](unsafe_from_address=Int(q8addr))
        var nb = (nr + RPW - 1) // RPW
        var rb = q8_row_bytes(nc)
        def wk4(wi: Int) capturing:
            var rs = wi * RPW; var re = rs + RPW
            if re > nr: re = nr
            for r in range(rs, re):
                var ro = r * rb
                var a0 = SIMD[DType.float32, W](0.0); var a1 = SIMD[DType.float32, W](0.0)
                var a2 = SIMD[DType.float32, W](0.0); var a3 = SIMD[DType.float32, W](0.0)
                var col = 0
                while col < nc:
                    var bo = ro + (col // QK) * QB; prefetch[](q + bo + QB)
                    var lo = Int(q.load(bo + 0)); var hi = Int(q.load(bo + 1))
                    ...
                    comptime for grp in range(4):
                        ...
                        a0 = wf.fma[FastMathFlag.FAST](x.load[width=W](0*nc + col + grp*8), a0)
                        a1 = wf.fma[FastMathFlag.FAST](x.load[width=W](1*nc + col + grp*8), a1)
                        a2 = wf.fma[FastMathFlag.FAST](x.load[width=W](2*nc + col + grp*8), a2)
                        a3 = wf.fma[FastMathFlag.FAST](x.load[width=W](3*nc + col + grp*8), a3)
                    col += QK
                o.store(0*nr + r, a0.reduce_add()); o.store(1*nr + r, a1.reduce_add())
                o.store(2*nr + r, a2.reduce_add()); o.store(3*nr + r, a3.reduce_add())
        parallelize[func=wk4](num_work_items=nb, num_workers=nw)

# ── Single-batch Q8_0 matmul (for per-expert dispatch) ──
@always_inline("nodebug")
def _mm_q8_single(q8addr: Int, x: UnsafePointer[Float32, MutExternalOrigin],
                   o: UnsafePointer[Float32, MutExternalOrigin],
                   nr: Int, nc: Int, nw: Int = 32):
    var q = UnsafePointer[UInt8, MutExternalOrigin](unsafe_from_address=Int(q8addr))
    var nb = (nr + RPW - 1) // RPW
...
def _mm_f32_batch(f32addr: Int, x: UnsafePointer[Float32, MutExternalOrigin],
    ...
    def wk(wi: Int) capturing:
        ...
        for r in range(rs, re):
            ...
            var c: Int = 0
            while c + W <= nc:
                var wv = w.load[width=W](r * nc + c)
                acc = acc + wv * x.load[width=W](c)
                c += W
            o.store(r, acc.reduce_add())
    parallelize[func=wk](num_work_items=nb, num_workers=nw)
...
def main() raises:
    ...
    for pos in range(max_gen):
        ...
        # Use prompt[pos] for prefill, or the token generated at (pos-1) for generation
        var emb = UnsafePointer[UInt8, MutExternalOrigin](unsafe_from_address=Int(w_emb))
        for bi in range(B):
            var src_pos = pos if pos < np else (pos - 1)
            var tok = Int(batch_toks.load(bi * MAX_SEQ + src_pos))
        ...
        for l in range(NL):
            ...
            if l % 2 == 0:
                ...
                # Router stage 1: gate_inp F32 256×2048 (per batch item)
                for bi in range(B):
                    _mm_f32_batch(Int(w_gi), bp + bi*NE, router_h + bi*N_RH, N_RH, NE, nw)
                for bi in range(B):
                    var rh = router_h + bi*N_RH
                    for i in range(N_RH): rh.store(i, rh.load(i) + gi_bias_p.load(i))

                _mm_q8_batch(Int(w_fg), router_h, router_h2, N_RH, N_RH, nw)
                for bi in range(B):
                    var rh2 = router_h2 + bi * N_RH
                    for i in range(N_RH): rh2.store(i, rh2.load(i) + fg_bias_p.load(i))
                silu_inplace(router_h2, B * N_RH)
                _mm_q8_batch(Int(w_rm2), router_h2, router_h, N_RH, N_RH, nw)
                for bi in range(B):
                    var rh = router_h + bi*N_RH
                    for i in range(N_RH): rh.store(i, rh.load(i) + rm2_bias_p.load(i))
                _mm_q8_batch(Int(w_rm4), router_h, scores, N_EXP+1, N_RH, nw)

                # Per-batch-item: softmax, expert selection, expert dispatch
                var per_exp_bytes_gate = FF * emb_rb
                var per_exp_bytes_down = F2 * emb_rb
                for bi in range(B):
                    var sc = scores + bi * (N_EXP + 1)
                    for i in range(N_EXP + 1): sc.store(i, sc.load(i) + rb_p.load(i))
                    ...
                    for i in range(N_EXP + 1):
                        var ef = exp(sc.load(i) - smaxf)
                        sc.store(i, ef); ssumf += ef
                    var inv_ss = 1.0 / ssumf
                    for i in range(N_EXP + 1): sc.store(i, sc.load(i) * inv_ss)

                    # Top-1 expert
                    var ec = 0
                    for i in range(1, N_EXP):
                        if sc.load(i) > sc.load(ec): ec = i
                    var exp_sum = Float32(0.0)
                    for i in range(N_EXP): exp_sum += sc.load(i)
                    var ew = sc.load(ec) / exp_sum

                    # Expert gate_up
                    var xb = bp + bi * NE
                    var exp_gate_up = Int(w_gu) + ec * per_exp_bytes_gate
                    _mm_q8_single(exp_gate_up, xb, gate_up_buf, FF, NE, nw)
                    ...
                    # Expert down (output to expert_out to avoid buffer overlap)
                    var exp_down = Int(w_de) + ec * per_exp_bytes_down
                    _mm_q8_single(exp_down, gate_up_buf, expert_out, F2, F2, nw)

                    # Residual with learned scaling
                    var rr_w_addr = wl.load(lw + 3); var rr_b_addr = wl.load(lw + 4)
                    var has_res_scale = (rr_w_addr != 0) and (rr_b_addr != 0)
                    var hp_bi = hp + bi * NE
                    for i in range(NE):
                        var out = expert_out.load(i) * ew; var x_val = hp_bi.load(i)
                        if has_res_scale:
