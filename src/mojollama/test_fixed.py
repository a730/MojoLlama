#!/usr/bin/env python3
"""Fixed test with corrected output weight pointer."""
...
# FIX: use _out_raw instead of _or
bc.wOut = ctypes.cast(e._out_raw, cv) if hasattr(e, '_out_raw') else cv(0)
print(f"wOut ptr: {bc.wOut}, outNR={bc.outNR}, outNC={bc.outNC}, outQuant={bc.outQuant}", flush=True)
