#!/usr/bin/env python3
"""TurboEngine v8 — C layer forward pass.

Eliminates ALL Python per-layer overhead by calling the entire
transformer layer computation as a single C function:
  RMS norm, QKV matmul, RoPE, KV cache, GQA attention,
  O projection + residual, RMS norm 2, Gate+Up, SiLU×Up, Down + residual

Expected: 80+ tok/s (eliminates the 67% Python overhead from v7)
"""
...
class LayerParams(ctypes.Structure):
    """C-compatible struct for passing layer weights to layer_forward."""
    _fields_ = [
        # Weight pointers (quantized)
        ("attn_q_raw", ctypes.POINTER(ctypes.c_uint8)),
        ("attn_k_raw", ctypes.POINTER(ctypes.c_uint8)),
        ("attn_v_raw", ctypes.POINTER(ctypes.c_uint8)),
        ("attn_out_raw", ctypes.POINTER(ctypes.c_uint8)),
        ("ffn_gate_raw", ctypes.POINTER(ctypes.c_uint8)),
        ("ffn_up_raw", ctypes.POINTER(ctypes.c_uint8)),
        ("ffn_down_raw", ctypes.POINTER(ctypes.c_uint8)),
        # Norm weights
        ("attn_norm_w", ctypes.POINTER(ctypes.c_float)),
        ("ffn_norm_w", ctypes.POINTER(ctypes.c_float)),
        # Quant types
        ("qt_q", ctypes.c_int), ("qt_k", ctypes.c_int), ("qt_v", ctypes.c_int),
        ("qt_out", ctypes.c_int), ("qt_gate", ctypes.c_int), ("qt_up", ctypes.c_int), ("qt_down", ctypes.c_int),
        # Dimensions
        ("nr_q", ctypes.c_int), ("nr_k", ctypes.c_int), ("nr_v", ctypes.c_int),
        ("nr_out", ctypes.c_int), ("nr_gate", ctypes.c_int), ("nr_up", ctypes.c_int), ("nr_down", ctypes.c_int),
        ("nc", ctypes.c_int),
        # Flags
        ("use_c_qkv", ctypes.c_int), ("use_c_gate_up", ctypes.c_int),
    ]
...
class TurboEngineV8:
    ...
    def _load_kernels(self):
        ...
        if os.path.exists(so_path):
            ...
            # Batch functions
            so.batch_qkv_omp.argtypes = [cu, cu, cu, cf, cf, cf, ci, ci, ci, ci, ci, ci]
        ...
        # C layer forward library (v8)
        layer_lib_path = os.path.join(kernel_dir, 'liblayer_forward.so')
        if os.path.exists(layer_lib_path):
            layer_lib = ctypes.CDLL(layer_lib_path)
            layer_lib.layer_forward.argtypes = [
                ctypes.POINTER(LayerParams),  # lp
                cf, cf, cf, cf,  # x, x_norm, residual, q
                cf, cf, cf,  # k, v, att_out
                cf, cf, cf, cf, cf,  # o_proj, gate, up, silu_gate, ffn_out
                cf, cf,  # kv_k, kv_v
                ci, ci, ci, ci, ci,  # pos, n_embd, n_head, n_kv_head, head_dim
                ci, ctypes.c_float, ci,  # n_ff, eps, kv_len
            ]
            layer_lib.layer_forward.restype = None
            self._layer_lib = layer_lib
        else:
            self._layer_lib = None
    ...
    def _preload_layer_weights(self):
        ...
        if out_info is not None:
            ...
            self._out_qt = ctypes.c_int(out_info[3])
    ...
    def _build_layer_params(self):
        """Build C LayerParams structs from Python LayerWeights."""
        self._c_layer_params = []
        for i, lw in enumerate(self._layers):
            lp = LayerParams()
            lp.attn_q_raw = lw.attn_q_raw
            lp.attn_k_raw = lw.attn_k_raw
            lp.attn_v_raw = lw.attn_v_raw
            lp.attn_out_raw = lw.attn_out_raw
            lp.ffn_gate_raw = lw.ffn_gate_raw
            lp.ffn_up_raw = lw.ffn_up_raw
            lp.ffn_down_raw = lw.ffn_down_raw
            lp.attn_norm_w = lw.attn_norm_w.ctypes.data_as(ctypes.POINTER(ctypes.c_float))
            lp.ffn_norm_w = lw.ffn_norm_w.ctypes.data_as(ctypes.POINTER(ctypes.c_float))
            lp.qt_q = lw.attn_q_qt.value if hasattr(lw.attn_q_qt, 'value') else lw.attn_q_qt
            lp.qt_k = lw.attn_k_qt.value if hasattr(lw.attn_k_qt, 'value') else lw.attn_k_qt
            lp.qt_v = lw.attn_v_qt.value if hasattr(lw.attn_v_qt, 'value') else lw.attn_v_qt
            lp.qt_out = lw.attn_out_qt.value if hasattr(lw.attn_out_qt, 'value') else lw.attn_out_qt
            lp.qt_gate = lw.ffn_gate_qt.value if hasattr(lw.ffn_gate_qt, 'value') else lw.ffn_gate_qt
            lp.qt_up = lw.ffn_up_qt.value if hasattr(lw.ffn_up_qt, 'value') else lw.ffn_up_qt
            lp.qt_down = lw.ffn_down_qt.value if hasattr(lw.ffn_down_qt, 'value') else lw.ffn_down_qt
            lp.nr_q = lw.attn_q_nr.value if hasattr(lw.attn_q_nr, 'value') else lw.attn_q_nr
            lp.nr_k = lw.attn_k_nr.value if hasattr(lw.attn_k_nr, 'value') else lw.attn_k_nr
            lp.nr_v = lw.attn_v_nr.value if hasattr(lw.attn_v_nr, 'value') else lw.attn_v_nr
            lp.nr_out = lw.attn_out_nr.value if hasattr(lw.attn_out_nr, 'value') else lw.attn_out_nr
            lp.nr_gate = lw.ffn_gate_nr.value if hasattr(lw.ffn_gate_nr, 'value') else lw.ffn_gate_nr
            lp.nr_up = lw.ffn_up_nr.value if hasattr(lw.ffn_up_nr, 'value') else lw.ffn_up_nr
            lp.nr_down = lw.ffn_down_nr.value if hasattr(lw.ffn_down_nr, 'value') else lw.ffn_down_nr
            lp.nc = lw.attn_q_nc.value if hasattr(lw.attn_q_nc, 'value') else lw.attn_q_nc
            lp.use_c_qkv = 1 if (lw.attn_q_use_c and lw.attn_k_use_c and lw.attn_v_use_c) else 0
            lp.use_c_gate_up = 1 if (lw.ffn_gate_use_c and lw.ffn_up_use_c) else 0
            self._c_layer_params.append(lp)

    def forward(self, token_id):
        """C layer forward pass — single ctypes call per layer."""
        N = self.n_embd; NH = self.n_head; NKH = self.n_kv_head; HD = self.head_dim
        NK = NH * HD; FF = self.n_ff; L = self.n_layers
        kern = self._kern; simd = self._simd; layer_lib = self._layer_lib
        eps_f = self._eps_f

        # Embedding lookup
        np.copyto(self._x, self.emb[token_id])

        # Use C layer forward if available and all kernels are C
        if layer_lib is not None:
            for i in range(L):
                lp = self._c_layer_params[i]
                kv_pos = int(self.kv_len[i])

                # Call the C layer forward function
                layer_lib.layer_forward(
                    ctypes.byref(lp),
                    self._p_x, self._p_x_norm, self._p_residual, self._p_q,
                    self._p_k, self._p_v, self._p_att_out,
                    self._p_o_proj, self._p_gate, self._p_up, self._p_silu_gate, self._p_ffn_out,
                    # KV cache for this layer — pointer to kv_k[layer, 0, 0]
                    self.kv_k[i].ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
                    self.kv_v[i].ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
                    ctypes.c_int(self.pos),        # pos
                    ctypes.c_int(N),               # n_embd
                    ctypes.c_int(NH),              # n_head
                    ctypes.c_int(NKH),             # n_kv_head
                    ctypes.c_int(HD),              # head_dim
                    ctypes.c_int(FF),              # n_ff
                    ctypes.c_float(self.eps),      # eps
                    ctypes.c_int(kv_pos),          # kv_len (before this token)
                )
                self.kv_len[i] += 1
        else:
            # Fallback: Python forward pass (same as v7)
            ...
            for i in range(L):
                ...
                if lw.attn_q_use_c and lw.attn_k_use_c and lw.attn_v_use_c:
                    kern.batch_qkv_omp(
                        ...
                        lw.attn_q_nc, lw.attn_k_qt, lw.attn_v_qt
                    )
        ...
        # Final norm + output projection (always in Python — tiny overhead)
