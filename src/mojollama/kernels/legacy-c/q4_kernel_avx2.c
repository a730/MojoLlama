/* Q4_0/Q4_1 matmul AVX2 kernel (single-threaded, for per-row threading).
...
/* Row-outer accumulation: total=0, accumulate blocks, write once per row. */
void q4_matmul_avx2(
    ...
    for (int r = start; r < end; r++) {
        ...
        for (int blk = 0; blk < bpr; blk++) {
            __m256 v[4]; decode(w + (r * bpr + blk) * type_size, v);
            int io = blk * 32;
            __m256 a = _mm256_mul_ps(v[0], _mm256_loadu_ps(x + io));
            a = _mm256_fmadd_ps(v[1], _mm256_loadu_ps(x + io + 8), a);
            a = _mm256_fmadd_ps(v[2], _mm256_loadu_ps(x + io + 16), a);
            a = _mm256_fmadd_ps(v[3], _mm256_loadu_ps(x + io + 24), a);
