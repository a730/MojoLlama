#!/usr/bin/env python3
"""Standalone MoE engine test — loads model, runs 1 forward step, prints logits."""
import signal, faulthandler, sys, os, time, ctypes, json
faulthandler.enable()
# Also register a signal handler for SIGSEGV
...
for attr, val in [('L',L),('N',N),('NH',NH),('NKH',NKH),('HD',HD),('FF',FF),('V',V),('eps',eps),
                 ...
                 ('gate_exp_quant',12),('up_exp_quant',12),('down_exp_quant',14),
                  ('emb_quant',0)]:
    setattr(bc, attr, val)

# Weight pointers from engine
...
# RoPE table - precompute using numpy
hd2 = HD // 2
max_ctx = 4096
...
# Run with all layers
token = 785  # "The" in Qwen3 tokenizer
...
# ── Run 5 more tokens for speed test ──
